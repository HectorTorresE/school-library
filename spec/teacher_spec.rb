...
RSpec.describe Teacher do
  teacher = Teacher.new('45', 'Math', 'hector', 645)
  ...
  it 'Converting an nstance of Teacher class to json ' do
    expect(teacher.to_json).to be == '{"json_class":"Teacher","data":["45","Math","hector",645]}'
  end

  it 'Converting a json string to an instance of Teacher class' do
    teacher = JSON.parse('{"json_class":"Teacher","data":["45","Math","hector",645]}', create_additions: true)
    expect(teacher).to be_an_instance_of(Teacher)
  end
  ...
  it 'Using decorators teacher name should be able to be capitalized' do
    ...
    expect(capitalize_decorator.correct_name).to be == 'Hector'
  end
end
