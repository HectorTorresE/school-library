...
RSpec.describe Student do
  before :each do
    @student = Student.new(10, Classroom.new('1B'), 'Hector')
  end
  ...
  it 'Create JSON string' do
    @student = Student.new('10', Classroom.new('1B'), 'Hector', 53, parent_permission: true)
    json = JSON.generate(@student)
    expect(json).to eq '{"json_class":"Student","data":["10",{"json_class":"Classroom","label":"1B"},"Hector",53,true]}'
  end

  it 'Creating a student instance from a json string' do
    student = JSON.parse(
      '{"json_class":"Student","data":["10",{"json_class":"Classroom","label":"1B"},"Hector",53,true]}',
